...
export default function Advertisement({ advertisement }) {
  ...
  return (
    <div>
      ...
      >
        <button
          onClick={() => handlAddFavorite(advertisement)}
          className="btn m-0 btn-add-favorites my-auto btn-sm py-0 px-1 small btn-outline-danger"
        >
          {getAdvertisement(advertisement) ? (
            <i className="fa my-auto fa-heart"></i>
          ) : (
            <i className="far my-auto pt-1 fa-heart"></i>
          )}
        </button>
        <Link to={`/advertisement/${advertisement.id}`}>
          <span className="avatar">{advertisement.jobTitle?.title[0]}</span>
          <span className="detail">
            ...
            <span className="title">{advertisement.jobTitle?.title}</span>
        ...
        <div className="card-footer py-1 bg-light">
          <div className="d-flex primary-color-text flex-wrap justify-content-between">
            <small>{advertisement?.numberOfOpenPositions} Kişi</small>
